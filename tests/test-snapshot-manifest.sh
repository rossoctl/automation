...
#     a reconstructed slug (discussion #62: owner may not equal org)
...
# Hermetic: fake enrolled repos via $REPOMAN_REPOS_FILE + $REPOMAN_CONFIG_FILE,
# real throwaway git clones under a temp $REPOS_DIR, fake openclaw/node on PATH.
...
git init -q "$REPOS_DIR/someowner/tool-one"
(
  cd "$REPOS_DIR/someowner/tool-one"
...
git init -q "$REPOS_DIR/someowner/tool-two"
(
  cd "$REPOS_DIR/someowner/tool-two"
...
if [ -f "$MANIFEST" ]; then
  ...
  one_origin=$(jq -r '.repos[] | select(.name=="someowner/tool-one") | .origin' "$MANIFEST")
  if [ "$one_origin" != "$NONACME_ORIGIN" ]; then
    echo "FAIL manifest: tool-one origin got [$one_origin], want [$NONACME_ORIGIN]"
    fail=1
  fi

  one_branch=$(jq -r '.repos[] | select(.name=="someowner/tool-one") | .branch' "$MANIFEST")
  if [ "$one_branch" != "feat/x" ]; then
    echo "FAIL manifest: tool-one branch got [$one_branch]"
    fail=1
  fi

  one_dirty=$(jq -r '.repos[] | select(.name=="someowner/tool-one") | .dirty' "$MANIFEST")
  if [ "$one_dirty" != "true" ]; then
    echo "FAIL manifest: tool-one dirty got [$one_dirty], want true"
    fail=1
  fi

  # origin must never be a reconstructed slug when a real origin exists.
  if jq -r '.repos[].origin' "$MANIFEST" | grep -Fq "github.com/someowner/tool-one"; then
    echo "FAIL manifest: origin was reconstructed (github.com/someowner/tool-one) instead of real remote"
    fail=1
  fi

  # tool-two: no origin remote -> empty origin string.
  two_origin=$(jq -r '.repos[] | select(.name=="someowner/tool-two") | .origin' "$MANIFEST")
