...
check_module org.sh repoman_config repoman_get_repos is_enrolled repoman_load_enrolled is_enrolled_in enrolled_clone_dirs validate_repos_dir
check_module repoman-requirements.sh repoman_parse_requirements

if [ "$fail" -eq 0 ]; then
  echo "PASS: all five modules source standalone with their functions defined"
  exit 0
fi
exit 1
